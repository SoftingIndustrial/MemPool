...
{                                                                             \
  if (chunksize(P) < MAX_SMALLBIN_SIZE || P->fd_samesize==P)                  \
  {                                                                           \
    FD = P->fd;                                                               \
    BK = P->bk;                                                               \
    ASSERT(FD->bk == P);                                                      \
  ...
  {                                                                           \
    FD = P->fd_samesize;                                                      \
    BK = P->bk_samesize;                                                      \
    ASSERT(FD->bk_samesize == P);                                             \
    ASSERT(BK->fd_samesize == P);                                             \
    FD->bk_samesize = BK;                                                     \
    BK->fd_samesize = FD;                                                     \
    if (P->fd != NULL)                                                        \
    {                                                                         \
...
void
Cyg_dlmalloc_init( cyg_uint8 *base, cyg_uint32 size )
{
    size_t front_misalign;
    unsigned correction;

#if defined(CYGDBG_MEMALLOC_ALLOCATOR_DLMALLOC_DEBUG)
    static int instances;
    if ( ++instances > 1 )
        CYG_FAIL( "Multiple dlmalloc instances not supported" );
#endif
    ...
    front_misalign = (size_t)chunk2mem(base) & MALLOC_ALIGN_MASK;

    if ( front_misalign > 0 ) {
        correction = (unsigned)(MALLOC_ALIGNMENT - front_misalign);
    } else {
        correction = 0;
    }
    ...
    if ( correction + 2*(unsigned)MALLOC_ALIGNMENT > (unsigned) size )
        ...
        set_head(top, ((size - correction) & ~MALLOC_ALIGN_MASK) | PREV_INUSE);
    }
}
...
cyg_uint8 *
Cyg_dlmalloc_try_alloc( cyg_uint32 bytes )
{
  ...
  if ((unsigned long)(nb) < (unsigned long)(bytes))
    return NULL;
  ...
  */
  ...
  if ( (block = idx2binblock(idx)) <= binblocks)
  {
    ...
    for (;;)
    {
      ...
      do
      {
        ...
        for (victim = last(bin); victim != bin; victim = victim->bk)
        {
          ...
          if (remainder_size >= (long)MINSIZE) /* split */
          {
            ...
            frontlink(remainder, (INTERNAL_SIZE_T)remainder_size, remainder_index, bck, fwd);
            check_malloced_chunk(victim, nb);
            actualAllocBytes += chunksize(victim);
            if(actualAllocBytes > maxAllocBytes)
            {
                maxAllocBytes = actualAllocBytes;
            }
            MALLOC_UNLOCK;
      ...
      while (first(q) == q)
      {
        if ( (startidx & (BINBLOCKWIDTH - 1)) == 0 || (startidx < 2) )
        {
          binblocks &= ~block;
          break;
        }
...
cyg_uint8 *
Cyg_dlmalloc_resize_alloc( cyg_uint8 *oldmem,
                           cyg_uint32 bytes,
                           cyg_uint32 *poldsize )
{
  ...
  if ((long)(oldsize) < (long)(nb))
  {
    ...
    if (next == top || !inuse(next))
    {
      ...
      if (next == top)
      {
        if ((long)(nextsize + newsize - MINSIZE) >= (long)(nb))
        {
          newsize += nextsize;
          top = chunk_at_offset(oldp, nb);
          set_head(top, (newsize - nb) | PREV_INUSE);
          set_head_size(oldp, nb);
          actualAllocBytes = actualAllocBytes - oldsize + chunksize(oldp);
          if(actualAllocBytes > maxAllocBytes)
          {
              maxAllocBytes = actualAllocBytes;
          }
    ...
    if (!prev_inuse(oldp))
    {
      ...
      if (next != 0)
      {
        ...
        if (next == top)
        {
          if ((long)(nextsize + prevsize + newsize - MINSIZE) >= (long)(nb))
          {
            unlink(prev, bck, fwd);
            newp = prev;
            ...
            actualAllocBytes = actualAllocBytes - oldsize + chunksize(newp);
            if(actualAllocBytes > maxAllocBytes)
            {
                maxAllocBytes = actualAllocBytes;
            }
 ...
 split:  /* split off extra room in old or expanded chunk */
  ...
  if (remainder_size >= (unsigned int)MINSIZE) /* split off remainder */
  {
    ...
    /* Adapt actual alloc (free will decrease actual alloc bytes again) */
    actualAllocBytes = actualAllocBytes + chunksize(remainder);
    Cyg_dlmalloc_free( chunk2mem(remainder) );
  }
  else
  {
...
} /* Cyg_dlmalloc_resize_alloc() */

/*---------------------------------------------------------------------------*/

/* Get memory pool status */
/* flags is a bitmask of requested fields to fill in. The flags are */
/* defined in mpcommon.h */
void
Cyg_dlmalloc_get_status( cyg_uint32 flags,
                         struct Cyg_Mempool_Status *status )
{
                     ...
                     CYG_MEMPOOL_STAT_TOTALALLOCATED|CYG_MEMPOOL_STAT_MAXFREE)))
    {
        ...
        for (i = 1; i < CYGPRI_MEMALLOC_ALLOCATOR_DLMALLOC_NAV; ++i) {
            b = bin_at(i);
            for (p = last(b); p != b; p = (i < MAX_SMALLBIN_SIZE/SMALLBIN_WIDTH ? p->bk :
                                           p->bk_samesize->bk != NULL ? p->bk_samesize->bk : p->bk_samesize)) {
#ifdef CYGDBG_MEMALLOC_ALLOCATOR_DLMALLOC_DEBUG
                check_free_chunk(p);
                for (q = next_chunk(p);
                     (q < top) && inuse(q) &&
    ...
    status->maxAllocBytes = maxAllocBytes;
    status->actualAllocBytes = actualAllocBytes;

    MALLOC_UNLOCK;
} /* Cyg_dlmalloc_get_status() */
